...
var AutoPager = function (info) {
  ...
  if (isFirefoxExtension()) {
    ...
    this.initHelp();
    GM_addStyle(
      '@media print{#autopagerize_icon, #autopagerize_help {display: none !important;}}'
    );
  ...
  var bottom =
    getElementPosition(this.insertPoint).top ||
...
AutoPager.prototype.initHelp = function () {
  ...
  for (var i in COLOR) {
    s +=
      '<div style="float:left;width:1em;height:1em;' +
      'margin:0 3px;background-color:' +
      COLOR[i] +
      ';' +
      '"></div><div style="margin:0 3px">' +
      i +
      '</div>';
  }
  s += '</div>';
  ...
  versionDiv.innerHTML =
    '<a href="' + URL + '">AutoPagerize</a> ver ' + VERSION;
  ...
  GM_addStyle(
    '#autopagerize_help a { color: #0f0; text-decoration: underline;}'
  );
};
...
AutoPager.prototype.updateIcon = function (state) {
  ...
  if (color) {
    if (isFirefoxExtension()) {
      ...
      chrome.extension
        .connect({ name: 'pageActionChannel' })
        .postMessage(color);
...
AutoPager.prototype.addPage = function (htmlDoc, page) {
  ...
  if (page[0] && /tr/i.test(page[0].tagName)) {
    ...
    var colNodes = getElementsByXPath(
  ...
  p.innerHTML =
    'page: <a class="autopagerize_link" href="' +
    this.requestURL.replace(/&/g, '&amp;') +
    '">' +
    ++this.pageNum +
    '</a>';
      ...
      function (event) {
        if (event.name === 'settings') {
          ...
          if (
            ...
            !isExclude(settings['exclude_patterns'])
          ) {
            launchAutoPager(SITEINFO);
...
if (
  location.href.match('^http://[^.]+.google.(?:[^.]{2,3}.)?[^./]{2,3}/.*(&fp=)')
) {
...
function getElementsByXPath(xpath, node) {
  var nodesSnapshot = getXPathResult(
    xpath,
    node,
